...
const regComposedActivity= new RegExp('(?<=/ca/)([^/]+)');
...
const moduloId = regModulo.test(url?.pathname)? url?.pathname?.match(regModulo)![0]:''
 const fragmentId = regFragment.test(url?.pathname)? url?.pathname?.match(regFragment)![0]:''
